...
{
    /// <summary>
    /// SerializeReference를 사용하는 Dictionary
    /// Value에는 다음값이 오면 안됩니다.(직렬화 불가)
    /// - Serializable이 아닌 클래스
    /// - UnityEngine.Object를 상속받는 클래스
    /// - C# 값 타입
    /// </summary>
    ...
    public class SerializableReferenceDictionary<TKey, TValue> : Dictionary<TKey, TValue>, ISerializationCallbackReceiver
    {
        ...
        [System.Serializable]
        public class SerializableReferenceKeyValuePair
        {
            [SerializeField] public TKey Key;
            [SerializeReference][SerializeReferenceDropdown] public TValue Value;
            
            public SerializableReferenceKeyValuePair(TKey key, TValue value)
            {
                Key = key;
                Value = value;
            }
        ...
        [SerializeField] private List<SerializableReferenceKeyValuePair> _items = new List<SerializableReferenceKeyValuePair>();
        ...
        public void OnBeforeSerialize()
        {
            ...
            {
                _items.Add(new SerializableReferenceKeyValuePair(kvp.Key, kvp.Value));
            }
    ...
    #if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(SerializableReferenceDictionary<,>), true)]
    public class SerializableReferenceDictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        private void DrawList(Rect rect, SerializedProperty items, ref int selectedIndex)
        {
            ...
            for (int i = 0; i < items.arraySize; i++)
            {
                ...
                var copiedEnum = elem.GetChildProperties().GetEnumerator();
                if(!copiedEnum.MoveNext())
                    continue;
                var key = copiedEnum.Current;
                Debug.Log($"Key Property: {key.propertyPath}");
                if(!copiedEnum.MoveNext())
                    continue;
                var val = copiedEnum.Current;
                Debug.Log($"Value Property: {val.propertyPath}");
                float keyH = EditorGUI.GetPropertyHeight(key, true);
                float valH = EditorGUI.GetPropertyHeight(val, true);
