...
{
    /// <summary>
    /// <see cref="CardSpec"/>을 기반으로 <see cref="LegacyCardState"/>를 생성하는 빌더.
    /// </summary>
    public sealed class LegacyCardStateBuilder
    {
        // 해당 속성의 기본값들.
        ...
        /// </summary>
        public LegacyCardState Build(CardSpec spec)
        {
            Clear();
            ...
            var state = new LegacyCardState(spec);
        ...
        private static LegacyCardState.SelectableValues<T> BuildSelectable<T>(
            T? defaultValue,
            IReadOnlyList<T> alternatives) where T : struct
        {
            var selectable = new LegacyCardState.SelectableValues<T>(defaultValue);
