...
namespace Cardevil.Card.Common.Core
{
    ...
    /// 카드의 플레이용 상태 데이터.
    /// </summary>
    public interface ILegacyCardState : IScoreSource
    {
        LegacyCardState.SelectableValues<CardColor> Colors { get; }
        LegacyCardState.SelectableValues<int> Numbers { get; }
        LegacyCardState.SelectableValues<Direction> Directions { get; }
        ...
        LegacyCardState.ValueSelectableType SelectableType { get; }
    ...
    /// </summary>
    [Serializable]
    public sealed class LegacyCardState : ILegacyCardState, IDeepClonable<LegacyCardState>
    {
        ...
        public bool ValueSelected
        {
            get
            {
                switch (spec.Type)
                {
                    ...
                    default: throw new ArgumentOutOfRangeException(nameof(LegacyCardState));
                }
            }
        }
        ...
        public LegacyCardState(CardSpec spec)
        {
            this.spec = spec;
        }

        public LegacyCardState DeepClone()
        {
            var clone = new LegacyCardState(spec);
