...
namespace Cardevil.Card.InStage
{
    ...
    {
        [SerializeReference] private List<INewCardState> hand = new();
        [SerializeReference] private List<INewCardState> selection = new();
        ...
        private readonly HashSet<INewCardState> handSet = new();
        private readonly HashSet<INewCardState> selectionSet = new();

        public IReadOnlyList<INewCardState> Hand => hand;
        public IReadOnlyList<INewCardState> Selection => selection;

        /// <summary>
        /// 손패 상에 위치한 순서대로 정렬된 선택 목록.
        /// 호출 시 새로운 객체를 반환함.
        /// </summary>
        public IReadOnlyList<INewCardState> SortedSelection => selection.OrderBy(s => hand.IndexOf(s)).ToList();

        public void Add(INewCardState state)
        {
            if (!handSet.Add(state)) return;
            hand.Add(state);
        }

        public void Insert(int index, INewCardState state)
        {
            if (!handSet.Add(state)) return;
            hand.Insert(index, state);
        }

        public void Remove(INewCardState state)
        {
        ...
        public void Select(INewCardState state)
        {
            if (selectionSet.Add(state))
                selection.Add(state);
        }

        public void Deselect(INewCardState state)
        {
            if (selectionSet.Remove(state))
                selection.Remove(state);
        }
        ...
        /// 정보를 저장하고 리스트에서 제거함.
        /// </summary>
        public void Detach(INewCardState state)
        {
            Remove(state);
        ...
        /// 정보를 바탕으로 리스트에 복구함.
        /// </summary>
        public void Reattach(INewCardState state)
        {
        ...
        public int IndexOf(INewCardState state) => hand.IndexOf(state);
        ...
        public void Sort(IComparer<INewCardState> comparer)
        {
        ...
        {
            [field: SerializeReference] public INewCardState Card { get; private set; }
            ...
            public InteractionData(INewCardState card, int originalIndex, bool isSelected)
            {
                Card = card;
        ...
        public void SetPointerDownData(INewCardState state)
        {
        ...
        public void SetDraggingData(INewCardState state)
        {
        ...
        public void SetDetachData(INewCardState state)
        {
